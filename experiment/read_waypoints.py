...
def main():
    ...
    filepath = "plot/kaust_bbox.geojson"
    kaust_bbox = read_geojson(filepath)
    src_bbox = tuple(kaust_bbox.bounds.iloc[0])  # Use the geometry bounds as source bbox

    tgt_bbox = (0, 0, 100, 100)      # Example target bounding box
    linestring_trajectories = project_to_bbox(linestring_trajectories, src_bbox, tgt_bbox)
