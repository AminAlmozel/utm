...
class myio:
    ...
    def import_delivery(self):
        filename = self.project + "delivery/*.geojson"
        list_of_files = glob.glob(filename)
        ls = np.zeros((1,2))
        list_of_df = []
        for filename in list_of_files:
        ...
        ls = np.delete(ls, 0, 0)
        # self.se_ls = np.concatenate((self.se_ls, ls), axis=0)
        ls_gdf = pd.concat(list_of_df, ignore_index=True)
        print("Imported delivery")
    ...
    def import_custom(self, name):
        filename = self.project + name + "/*.geojson"
        list_of_files = glob.glob(filename)
        ls = np.zeros((1,2))
        list_of_df = []
        for filename in list_of_files:
    ...
    def import_forbidden(self):
        ...
        for filename in list_of_files:
            ...
            for i in range(N):
                fa_temp[i, 0] = gdf[i].x
                fa_temp[i, 1] = gdf[i].y
            fa = np.concatenate((fa, fa_temp), axis=0)
        fa = np.delete(fa, 0, 0)

        fa_gdf = pd.concat(list_of_df, ignore_index=True)
    ...
    def write_path(path, name, color):
        traj = myio.path_to_traj(path)
        myio.write_traj(traj, name, color)

    def write_traj(traj, name, color):
        s_line = myio.traj_to_linestring(traj)
